        ...
        <main className="ml-0 md:ml-64 p-6 md:p-10 min-h-screen">
          ...
          <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-10">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-xl sm:text-2xl font-bold text-[#001360] mb-0.5">Manajemen Laporan</h1>
                <p className="text-sm text-[#807667]">Kelola, verifikasi, dan tindak lanjuti aduan infrastruktur dari masyarakat.</p>
              </div>
            </div>
            <div className="flex items-center shrink-0">
              <span className="inline-flex items-center px-4 py-1.5 rounded-full text-xs font-semibold text-[#001360] bg-[#001360]/10 border border-[#001360]/15 uppercase tracking-wider">
          ...
          <section className="legacy-card p-6 rounded-2xl mb-6">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              {/* Search */}
              <div>
                <label className="block text-sm font-semibold text-[#1C1B18] mb-2">Cari Laporan</label>
              ...
              {/* Category */}
              <div>
                <label className="block text-sm font-semibold text-[#1C1B18] mb-2">Kategori</label>
                <select
                  value={selectedKategori}
                  onChange={e => setSelectedKategori(e.target.value)}
                  className="w-full px-4 py-2.5 bg-white border border-[#D3C5B1] rounded-xl focus:ring-2 focus:ring-[#001360]/20 focus:border-[#001360] outline-none text-sm text-[#1C1B18] font-medium"
                  style={{ minHeight: '48px' }}
                >
              ...
              {/* Status */}
              <div>
                <label className="block text-sm font-semibold text-[#1C1B18] mb-2">Status</label>
                <select
              ...
              <div className="flex items-end">
                <button
                  ...
                  className="w-full bg-[#F6F3EC] border border-[#D3C5B1] hover:bg-white text-[#1C1B18] font-semibold py-3 px-5 rounded-[28px] flex items-center justify-center gap-2 transition-all active:scale-[0.98] text-sm cursor-pointer min-h-[48px]"
                >
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-[#E5E2E1] text-xs">
                  {loading ? (
                    ...
                    <tr>
                      <td colSpan={5} className="text-center py-12 text-[#807667]">Tidak ada laporan aduan yang cocok.</td>
                    </tr>
                  ) : (
                    filteredLaporan.map(aduan => (
                      <tr key={aduan.id} className="hover:bg-[#F6F3EC] transition-colors">
                        ...
                        <td className="px-6 py-4 text-[#4E4639] max-w-[240px] truncate" title={aduan.lokasi}>{aduan.lokasi}</td>
                        <td className="px-6 py-4">
                        ...
                        <td className="px-6 py-4 text-right flex items-center justify-end gap-2.5">
                          <Link
                            ...
                            className="bg-white hover:bg-[#001360] text-[#001360] hover:text-white border border-[#D3C5B1] hover:border-[#001360] px-4 py-2 rounded-[28px] text-xs font-semibold transition-all shadow-sm inline-flex items-center gap-1 cursor-pointer min-h-[36px]"
                          >
                          ...
                          {aduan.status === 'selesai' && (
                            <button
                              ...
                              className="bg-red-50 hover:bg-red-600 text-red-600 hover:text-white border border-red-200 hover:border-red-600 p-2 rounded-[28px] text-xs font-bold transition-all shadow-sm inline-flex items-center justify-center cursor-pointer active:scale-95 w-9 h-9 min-h-0 min-w-0"
